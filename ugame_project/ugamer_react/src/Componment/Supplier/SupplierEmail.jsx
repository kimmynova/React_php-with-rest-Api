...
const SupplierEmail = () => {
    ...
    useEffect(() => {
        if (supid) {
            setIsLoading(true);
            fetch(`http://localhost/ugame_project/ugamerphp/Supplier/supplier_phone_And_email.php/${supid}`)
                .then((response) => response.json())
                .then((data) => {
                   
                    console.log(data.employeeemailUpdate); // Log the 'employeephoneUpdate' property of the data object
                    
                    setTimeout(() => {
                       
                        setSupplieremail(data.employeeemailUpdate);
                        setIsLoading(false);
                    }, 1000);
                })
                .catch((err) => {
                ...
                <table className='w-full border-y'>
                    ...
                    <tbody className='text-center'>
                        {error ? (
                            ...
                            supplieremail.map((supplieremailData, index) => (
                                <tr
                                    className='dark:bg-[#3f4042] lg:mx-w-full md:text-xl sm:text-sm grid grid-cols-3 justify-center items-center border'
                                    key={`supplier-email${index}`}>
                                    <td className='flex pt-2 items-center justify-center'>{supplieremailData.supid}</td>
                                    <td className='flex pt-2 items-center justify-center md:pr-5 sm:pr-5'>{supplieremailData.supemail}</td>
