...
const Print_invoice = (
    { Employee, Invno, productitems, setShowInvoice, customer, Date, subtotal, Grandtotal,Remark,Exchange,Remaining
    ...
    const savehandle = async () => {
        try {
          ...
          const response = await fetch('http://localhost/ugame_project/ugamerphp/Invoice/invoice.php', {
            method: 'POST',
            headers: {
            ...
            if (responseData.success) {
              ...
              setTimeout(() => {
                ...
                if (Array.isArray(responseData.result) && responseData.result.length > 0) {
                    responseData.result.forEach(itemError => {
                      alert(itemError);
    ...
    return (
        <>
            ...
            <div className='border bg-white text-black w-[600px] h-full absolute top-5 right-0 left-0 bottom-9 m-auto shadow-[rgba(50,_50,_105,_0.15)_0px_2px_5px_0px,_rgba(0,_0,_0,_0.05)_0px_1px_1px_0px]'>
                ...
                <h1 className=' text-blue-500 font-bold text-3xl text-center'>Invoice</h1>
                    <table>
                        <thead>
